...
def search_api(query, pages=int(RESULT_COUNT / 10)):
    ...
    for i in range(0, pages):
        ...
        try:
            ...
            response.raise_for_status()  # Raise an exception for bad responses (e.g., 404, 500, etc.)
            data = response.json()
            if "items" in data:
                results += data["items"]
            else:
                print("No 'items' key found in the API response.")
        except RequestException as e:
            print("RequestException:", e)
            # Handle the error accordingly, e.g., retrying, logging, etc.
        except ValueError as e:
            print("ValueError:", e)
            # Handle the error accordingly, e.g., incorrect JSON format, etc.
